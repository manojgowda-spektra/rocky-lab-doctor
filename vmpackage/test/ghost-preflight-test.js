...
check('the glow reveal (onArrive) is what triggers it, and the CSS is click-through', () => {
  // The trigger is the REVEAL, not a particular inline closure. The callback passed to
  // LabPilotRocky.guide() is reveal(), which also fires on a 1.2 s deadline so the glow is
  // never withheld by a throttled animation frame — so assert that the reveal path calls
  // ghostFly, rather than that one specific line does. Asserting on the literal callback
  // broke the moment the deadline was added, while the behaviour was still correct.
  const revealFn = /function reveal\(\)\s*\{[\s\S]{0,400}?\n      \}/.exec(overlaySrc);
  assert.ok(revealFn, 'overlay.guide no longer has a reveal() path');
  assert.ok(/ghostFly\(/.test(revealFn[0]),
    'the glow reveal does not launch the ghost cursor');
  assert.ok(/LabPilotRocky\.guide\(element[^\n]*reveal\)/.test(overlaySrc),
    'reveal() is not what Rocky calls on arrival');
